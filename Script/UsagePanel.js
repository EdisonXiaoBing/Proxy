...
(async () => {
    ...
    if (!resetDayLeft && !expireDaysLeft) {
      ...
      content.push(`Data has been used${percentage}%`);
    } else {
      if (resetDayLeft && expireDaysLeft) {
        content.push(`Resets in ${resetDayLeft} days，Expires in ${expireDaysLeft} days`);
  ...
  function getUserInfo(url) {
    ...
    return new Promise((resolve, reject) =>
      $httpClient.get(request, (err, resp) => {
        ...
        reject("The link response header does not have traffic information.");
      })
    );
  }
