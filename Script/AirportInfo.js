(async () => {
  ...
  let content = [`Used: ${toPercent(used, total)} \t|  Remaining: ${toMultiply(total, used)}`];

  if (resetDayLeft || expire) {
    if (resetDayLeft && expire && expire !== "false") {
      if (/^[\d.]+$/.test(expire)) expire *= 1000;
      content.push(`Reset: ${resetDayLeft} days \t|  ${formatTime(expire)}`);
    } else if (resetDayLeft && !expire) {
      content.push(`Reset: ${resetDayLeft} days`);
    } else if (!resetDayLeft && expire) {
      if (/^[\d.]+$/.test(expire)) expire *= 1000;
